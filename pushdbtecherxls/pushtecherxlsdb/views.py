...
from pushtecherxlsdb.functions.functions import handle_uploaded_file
from pushtecherxlsdb.functions.pushdatabasefromteacherxls import PushDatabaseinCloudfromXLS
    ...
    if req.method == "POST":
        ...
        if(len(req.FILES) > 0):
            ...
            if(len(FileName) > 0 and (FileName.find('.xls') > -1 or FileName.find('.xlsx') > -1)):
                InputPath = os.getcwd()+'/pushtecherxlsdb/static/upload/'+str(FileName)
                print('Your Input path file = ', InputPath)
                pushstatus = PushDatabaseinCloudfromXLS(InputPath)
                print('Push status = ', pushstatus)
                FileName = str(FileName)
                samplepath = str('../static/output/sample1T.xlsx')
